class Pendulos extends THREE.Object3D {
  ...
  createGUI (gui) {
    ...
    this.guiControls = new function () {
      ...
      this.velocidad1=0.05;
      this.velocidad2=0.05;

      this.PendMOnOff=false;
      this.PendmOnOff=false;
      this.cambio1=false;
      this.cambio2=false;
    ...
    folder3.add (this.guiControls, 'PendMOnOff').name ('Animación pendulo Mayor: ');
    folder3.add(this.guiControls,'velocidad1',0.05, 1, 0.05).name ('Velocidad Rotación Péndulo Mayor').listen();
    folder3.add (this.guiControls, 'PendmOnOff').name ('Animación pendulo Menor: ');
    folder3.add(this.guiControls,'velocidad2',0.05, 1, 0.05).name ('Velocidad Rotación Péndulo Menor').listen();
  ...
  update () {
    ...
    if(this.guiControls.PendMOnOff){
      if(this.rotation.z>=1) this.cambio1=true;
      if(this.rotation.z<=-1) this.cambio1=false;

      if(this.cambio1)
      this.rotation.z-= this.guiControls.velocidad1;
      else
      this.rotation.z+= this.guiControls.velocidad1;
    ...
    if(this.guiControls.PendmOnOff){
      if(this.boxInf.rotation.z>=1) this.cambio2=true;
      if(this.boxInf.rotation.z<=-1) this.cambio2=false;

      if(this.cambio2)
      this.boxInf.rotation.z-= this.guiControls.velocidad2;
      else
      this.boxInf.rotation.z+= this.guiControls.velocidad2;
